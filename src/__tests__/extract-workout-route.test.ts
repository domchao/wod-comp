...
import { POST } from "@/app/api/extract-workout/route";

function makeRequest(body: Record<string, unknown>) {
  return new Request("http://localhost/api/extract-workout", {
    method: "POST",
...
describe("POST /api/extract-workout", () => {
  it("returns 400 when base64 is missing", async () => {
    ...
    expect(await res.json()).toEqual({ error: "Missing image data" });
  });
  ...
  it("returns extracted workout fields from a successful Gemini response", async () => {
    ...
    const res = await POST(makeRequest({ base64: "AAAA", mimeType: "image/jpeg" }));
  ...
  it("passes the image inline data to Gemini", async () => {
    mockGenerateContent.mockResolvedValue({ text: "{}" });

    await POST(makeRequest({ base64: "IMAGEDATA", mimeType: "image/png" }));
