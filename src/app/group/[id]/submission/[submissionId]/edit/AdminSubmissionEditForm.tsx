...
import { adminUpdateSubmission } from "./actions";
import { decodeRoundsReps } from "@/lib/submissions";
...
export function AdminSubmissionEditForm({
  ...
  const [state, action, pending] = useActionState<State, FormData>(adminUpdateSubmission, null);
  const [rx, setRx] = useState(submission.rx);
  ...
  const existingMins = Math.floor(submission.value / 60);
  const existingSecs = submission.value % 60;
  const existingDecoded = isRoundsAndReps ? decodeRoundsReps(submission.value) : null;
    ...
    <form action={action} className="space-y-5">
      ...
      <div className="rounded-lg border border-zinc-200 p-4 space-y-1">
        ...
        <p className="text-xs text-zinc-400 pt-1">
          Editing <span className="font-medium text-zinc-600">{memberName}</span>&apos;s submission
        </p>
      </div>
          ...
          <div className="flex items-center gap-2">
            <div className="flex-1 space-y-1">
              ...
              <input
                ...
                min="0"
                step="1"
                defaultValue={existingDecoded?.rounds ?? 0}
                placeholder="0"
                className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900"
              />
            </div>
            ...
            <div className="flex-1 space-y-1">
              ...
              <input
                ...
                min="0"
                step="1"
                defaultValue={existingDecoded?.reps ?? 0}
                placeholder="0"
                className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900"
              />
            </div>
